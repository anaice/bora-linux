...
configurar_voxtype() {
    ...
    for pkg in $VOXTYPE_AUR_PKGS voxtype; do
        if pacman -Q "$pkg" &>/dev/null; then
            log_ok "$pkg ($L_ALREADY_INSTALLED)"
        else
            run_cmd "$L_INSTALLING $pkg (AUR)" \
                "sudo -u $USUARIO_REAL yay -S --noconfirm $pkg" || {
                    [ "$pkg" = "voxtype" ] && { log_warn "$L_VOXTYPE_FAIL"; return 1; }
                    true
                }
        fi
    done
    ...
    # Systemd overrides: GPU (if Vulkan available) + dotool layout (BR ABNT2)
    local VT_OVERRIDE_DIR="$HOME_USUARIO/.config/systemd/user/voxtype.service.d"
    mkdir -p "$VT_OVERRIDE_DIR"
    {
        echo "[Service]"
        echo 'Environment="DOTOOL_XKB_LAYOUT=br"'
        echo 'Environment="DOTOOL_XKB_VARIANT=abnt2"'
        if command -v vulkaninfo &>/dev/null || pacman -Q vulkan-icd-loader &>/dev/null; then
            ...
            lspci | grep -iq nvidia && echo 'Environment="VOXTYPE_VULKAN_DEVICE=nvidia"'
        fi
    } > "$VT_OVERRIDE_DIR/bora.conf"
    ...
    # 3. Hotkey + notification config (sed-edit the default config generated by voxtype)
    local HOTKEY
    ...
    if [ -f "$VOXTYPE_CONF" ]; then
        case "$HOTKEY_NAME" in
            Ctrl+Shift+H)
                sed -i 's|^key = .*|key = "H"|' "$VOXTYPE_CONF"
                sed -i 's|^modifiers = .*|modifiers = ["LEFTCTRL", "LEFTSHIFT"]|' "$VOXTYPE_CONF"
                ;;
            ScrollLock)
                sed -i 's|^key = .*|key = "SCROLLLOCK"|' "$VOXTYPE_CONF"
                sed -i 's|^modifiers = .*|modifiers = []|' "$VOXTYPE_CONF"
                ;;
            Pause)
                sed -i 's|^key = .*|key = "PAUSE"|' "$VOXTYPE_CONF"
                sed -i 's|^modifiers = .*|modifiers = []|' "$VOXTYPE_CONF"
                ;;
        esac
        sed -i 's/^on_transcription = true/on_transcription = false/' "$VOXTYPE_CONF"
    fi
