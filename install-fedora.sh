...
# ============================================================
#  install-fedora.sh — Interactive setup for Fedora (GNOME / KDE)
#  Usage: sudo ./install-fedora.sh [-v|--verbose]
#  Everything via DNF + Flatpak
# ============================================================
...
menu_principal() {
    ...
    ETAPAS=$(gum choose --no-limit --height=50 \
        --header="  $L_MAIN_DESC" \
        --selected="repos  $L_STEP_REPOS","dnf  $L_STEP_DNF","flatpak  $L_STEP_FLATPAK","fonts  $L_STEP_FONTS","scripts  $L_STEP_SCRIPTS","mise  $L_STEP_MISE","lazyvim  $L_STEP_LAZYVIM","starship  $L_STEP_STARSHIP","zsh  $L_STEP_ZSH","ajustes  $L_STEP_TWEAKS" \
...
menu_dnf() {
    checklist "$L_DNF_TITLE" "$L_DNF_DESC" 38 78 28 \
        "gnupg2"                 "$L_APT_gpg"              ON \
        "curl"                   "$L_APT_curl"             ON \
        "wget"                   "$L_APT_wget"             ON \
        "zsh"                    "$L_APT_zsh"              ON \
        ...
        "tealdeer"               "$L_APT_tldr"             ON \
        "libyaml-devel"          "$L_APT_libyaml"          ON \
        "libxcb-cursor"          "$L_APT_libxcb"           ON \
        "mesa-libOpenGL"         "$L_APT_libopengl"        ON \
        "autoconf"               "$L_APT_autoconf"         ON \
        "openssl-devel"          "$L_APT_libssl"           ON \
        "ncurses-devel"          "$L_APT_libncurses"       ON
}
...
menu_cleanup() {
    checklist "$L_CLEANUP_TITLE" "$L_CLEANUP_DESC" 22 78 10 \
        "conflicting-repos" "$L_CLEANUP_DNF_REPOS"  ON \
        "flatpak-old"       "$L_CLEANUP_FLATPAK"     ON \
        "dnf-cache"         "$L_CLEANUP_DNF_CACHE"   ON \
...
executar_cleanup() {
    ...
    # Conflicting repos in /etc/yum.repos.d/
    if cleanup_selected "conflicting-repos"; then
        local DUPLICATAS=0
        local REPO_DIR="/etc/yum.repos.d"
        # Docker duplicates
        local docker_count
        docker_count=$(ls "$REPO_DIR"/docker*.repo 2>/dev/null | wc -l)
        if [ "$docker_count" -gt 1 ]; then
            # Keep only docker-ce.repo
            for f in "$REPO_DIR"/docker*.repo; do
                [ "$(basename "$f")" = "docker-ce.repo" ] && continue
                log_warn "$L_CLEANUP_DUP_REMOVED: $(basename "$f")"
                rm -f "$f"
    ...
    # DNF cache + autoremove
    if cleanup_selected "dnf-cache"; then
        run_cmd "$L_CLEANUP_DNF_CLEAN" \
            "dnf clean all && dnf autoremove -y" || true
    fi
...
atualizar_sistema() {
    ...
    run_cmd "$L_UPGRADE_UPGRADE" "dnf upgrade -y --refresh"
    run_cmd "$L_UPGRADE_AUTOREMOVE" "dnf autoremove -y"
}
...
configurar_repositorios() {
    ...
    run_cmd "$L_REPOS_DEPS" \
        "dnf install -y ca-certificates curl wget" || true

    # Helper to clean duplicate repos
    limpar_repo_duplicado() {
        local nome="$1"
        rm -f /etc/yum.repos.d/${nome}*.repo
    }

    # 1PASSWORD
    limpar_repo_duplicado "1password"
    run_cmd "Repo: 1Password" \
        "rpm --import https://downloads.1password.com/linux/keys/1password.asc && cat > /etc/yum.repos.d/1password.repo <<'REOF'
[1password]
...
REOF" || true
    ...
    # BRAVE
    limpar_repo_duplicado "brave"
    run_cmd "Repo: Brave Browser" \
        "rpm --import https://brave-browser-rpm-release.s3.brave.com/brave-core.asc && cat > /etc/yum.repos.d/brave-browser.repo <<'REOF'
[brave-browser]
name=Brave Browser
...
REOF" || true
    ...
    # CHROME
    limpar_repo_duplicado "google-chrome"
    run_cmd "Repo: Google Chrome" \
        "rpm --import https://dl.google.com/linux/linux_signing_key.pub && cat > /etc/yum.repos.d/google-chrome.repo <<'REOF'
[google-chrome]
name=Google Chrome
...
REOF" || true
    ...
    # DOCKER
    limpar_repo_duplicado "docker"
    run_cmd "Repo: Docker" \
        "dnf config-manager addrepo --from-repofile=https://download.docker.com/linux/fedora/docker-ce.repo 2>/dev/null || dnf config-manager --add-repo https://download.docker.com/linux/fedora/docker-ce.repo 2>/dev/null" || true

    # MISE
    limpar_repo_duplicado "mise"
    run_cmd "Repo: Mise" \
        "rpm --import https://mise.jdx.dev/gpg-key.pub && cat > /etc/yum.repos.d/mise.repo <<'REOF'
[mise]
name=mise
...
REOF" || true
    ...
    # EZA — already in Fedora repos, no extra repo needed

    # ANYDESK
    limpar_repo_duplicado "anydesk"
    run_cmd "Repo: AnyDesk" \
        "rpm --import https://keys.anydesk.com/repos/RPM-GPG-KEY && cat > /etc/yum.repos.d/anydesk.repo <<'REOF'
[anydesk]
name=AnyDesk Fedora
...
REOF" || true
    ...
    run_cmd "Repo: Ulauncher" \
        "dnf copr enable -y atim/ulauncher 2>/dev/null" || true

    run_cmd "$L_REPOS_UPDATE" "dnf makecache" || \
...
# ==========================================
instalar_dnf() {
    section "$L_DNF_SECTION"

    local PACOTES_DNF
    PACOTES_DNF=$(menu_dnf) || { log_skip "$L_DNF_NONE"; return 0; }

    # Enable RPM Fusion if vlc or mangohud selected
    if echo "$PACOTES_DNF" | grep -qE 'vlc|mangohud'; then
        run_cmd "$L_RPMFUSION_ENABLE" \
            "dnf install -y https://mirrors.rpmfusion.org/free/fedora/rpmfusion-free-release-\$(rpm -E %fedora).noarch.rpm https://mirrors.rpmfusion.org/nonfree/fedora/rpmfusion-nonfree-release-\$(rpm -E %fedora).noarch.rpm 2>/dev/null" || true
    fi

    local FALHAS=()
    for pacote in $PACOTES_DNF; do
        if rpm -q "$pacote" &>/dev/null; then
            log_ok "$pacote ($L_ALREADY_INSTALLED)"
        else
            run_cmd "$L_INSTALLING $pacote" "dnf install -y $pacote" || FALHAS+=("$pacote")
        fi
    done

    # No symlinks needed on Fedora (bat and fd are already correct names)
...
DTEOF
                ...
                ;;
            tabby)
                if command -v tabby &>/dev/null || rpm -q tabby-terminal &>/dev/null 2>&1; then
                    log_ok "Tabby ($L_ALREADY_INSTALLED)"
                else
                    local TABBY_VERSION TABBY_RPM
                    TABBY_VERSION=$(curl -s "https://api.github.com/repos/Eugeny/tabby/releases/latest" | grep -oP '"tag_name": "v\K[^"]+')
                    if [ -n "$TABBY_VERSION" ]; then
                        TABBY_RPM="/tmp/tabby.rpm"
                        run_cmd "$L_INSTALLING Tabby $TABBY_VERSION" \
                            "wget -q 'https://github.com/Eugeny/tabby/releases/download/v${TABBY_VERSION}/tabby-${TABBY_VERSION}-linux-x64.rpm' -O '$TABBY_RPM' && dnf install -y '$TABBY_RPM'; rm -f '$TABBY_RPM'" || true
                    else
    ...
    for lang in $LANGS; do
        case "$lang" in
            ...
            rust)
                ...
                ;;
            elixir)
                run_cmd "$L_MISE_ERLANG_DEPS" "dnf install -y autoconf openssl-devel ncurses-devel" || true
...
configurar_zsh_completo() {
    ...
    if ! command -v zoxide &>/dev/null; then
        run_cmd "$L_ZSH_ZOXIDE" "dnf install -y zoxide 2>/dev/null || { curl -sS https://raw.githubusercontent.com/ajeetdsouza/zoxide/main/install.sh | bash; }" || true
    else
...
configurar_teclado() {
    ...
    # Install uim
    run_cmd "$L_KEYBOARD_UIM" "dnf install -y uim"
...
limpeza_e_ajustes() {
    ...
    if [ "$XDG_CURRENT_DESKTOP" = "GNOME" ] || pgrep -x gnome-shell &>/dev/null; then
        local GNOME_CONF="$SCRIPT_DIR/configs/gnome/gnome.dconf"
        if [ -f "$GNOME_CONF" ]; then
            echo ""
            if gum confirm "$L_TWEAKS_GNOME_CONFIRM"; then
                sudo -u "$USUARIO_REAL" dbus-launch dconf load /org/gnome/ < "$GNOME_CONF"
                log_ok "$L_TWEAKS_GNOME_DONE"
            else
                log_skip "$L_TWEAKS_GNOME_SKIP"
            fi
        fi
    fi
...
main() {
    clear
    echo ""
    echo -e "${BOLD}${VERDE}  bora-linux (Fedora)${NC} — ${CINZA}$USUARIO_REAL${NC}"
